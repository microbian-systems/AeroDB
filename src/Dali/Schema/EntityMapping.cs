...
/// <summary>
/// Non-generic base for storing entity mappings in <see cref="SchemaOptions.EntityMappings"/>.
/// Public because <see cref="EntityMapping{T}"/> inherits from it.
/// </summary>
...
/// <summary>
/// Fluent API for entity-level schema configuration (indices, tenancy, etc.).
/// Accessed via <c>StoreOptions.Schema.ForEntity&lt;T&gt;()</c>.
/// Use this for <see cref="IEntity{TId}"/> types that do not extend <see cref="SurrealDb.Net.Models.Record"/>.
/// </summary>
    ...
    where T : class
{
    ...
    /// <summary>
    /// Sets the schema mode for this entity type (SCHEMAFULL vs SCHEMALESS).
    /// Default is <see cref="Dali.SchemaMode.Strict"/> (SCHEMAFULL).
    /// </summary>
    public EntityMapping<T> SetSchemaMode(Dali.SchemaMode mode)
    {
    ...
    /// </summary>
    public EntityMapping<T> Index<TProp>(Expression<Func<T, TProp>> property, Action<IndexOptions>? configure = null)
    {
    ...
    /// </summary>
    public EntityMapping<T> UniqueIndex<TProp>(Expression<Func<T, TProp>> property)
    {
        var member = ExtractMember(property);
    ...
    /// </summary>
    public EntityMapping<T> CompositeIndex(params Expression<Func<T, object>>[] properties)
    {
    ...
    /// </summary>
    public EntityMapping<T> UniqueCompositeIndex(params Expression<Func<T, object>>[] properties)
    {
        var columns = properties.Select(p => ExtractMember(p).Name).ToArray();
        Indices.Add(new IndexDefinition
        {
    ...
    public EntityMapping<T> FullTextIndex<TProp>(
    ...
    public EntityMapping<T> VectorIndex<TProp>(
        Expression<Func<T, TProp>> property,
        int dimension,
    ...
    public EntityMapping<T> HybridSearch(
    ...
    /// <summary>
    /// Marks this entity type as multi-tenanted (tenant_id filter applied).
    /// </summary>
    public EntityMapping<T> MultiTenanted()
    {
