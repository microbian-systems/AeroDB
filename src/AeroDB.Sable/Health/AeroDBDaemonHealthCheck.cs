...
/// <summary>
/// ASP.NET Core health check for the AeroDB.Sable async daemon.
...
public class AeroDBDaemonHealthCheck : IHealthCheck
{
    ...
    {
        if (_daemon is null)
            return Task.FromResult(HealthCheckResult.Healthy("AeroDB.Sable daemon not configured."));

        var health = _daemon.Health;

        if (!health.IsRunning)
            return Task.FromResult(HealthCheckResult.Unhealthy("AeroDB.Sable daemon is stopped."));
