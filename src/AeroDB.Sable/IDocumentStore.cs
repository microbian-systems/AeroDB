...
/// <summary>The primary entry point for the AeroDB.Sable document store. Provides access to query sessions, document sessions, event store, background daemon, and advanced SDK features.</summary>
public interface IDocumentStore : IAsyncDisposable
{
    ...
    /// <summary>Advanced/low-level SDK access for scenarios AeroDB.Sable doesn't abstract.</summary>
...
public interface IQuerySession : IAsyncDisposable
{
    ...
    /// Delegates to the new <see cref="ILiveQuerySession"/> fluent API internally.
