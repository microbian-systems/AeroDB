    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(compilationAndTypes, static (ctx, source) =>
        {
            ...
            if (relationshipCatalog is not null)
                ctx.AddSource("AeroDB.GeneratedRelationshipCatalog.g.cs", relationshipCatalog);
    ...
    private static string? GenerateRelationshipCatalog(IReadOnlyList<INamedTypeSymbol> validTypes)
    {
        ...
        foreach (var sourceType in validTypes)
        {
            foreach (var property in sourceType.GetMembers().OfType<IPropertySymbol>())
            {
                ...
                candidates.Add(
                    $"            new global::AeroDB.RelationshipCandidate(\"{Escape(sourceType.Name)}\", \"{Escape(target.Name)}\", \"{Escape(property.Name)}\", \"{Escape(targetId.Name)}\", global::AeroDB.RelationshipStorageKind.ScalarForeignKey, global::AeroDB.RelationshipCardinality.One, false)");
            }
        }
        ...
        sb.AppendLine("    {");
        sb.AppendLine("        global::AeroDB.Metadata.MetadataRegistry.RegisterRelationshipCandidates(");
        sb.Append(string.Join(",\n", candidates));
        sb.AppendLine();
        sb.AppendLine("        );");
        sb.AppendLine("    }");
