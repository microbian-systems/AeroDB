...
public class AeroDBDaemonHealthCheck : IHealthCheck
{
    ...
    {
        if (_daemon is null)
            return Task.FromResult(HealthCheckResult.Healthy("AeroDB daemon not configured."));

        var health = _daemon.Health;

        if (!health.IsRunning)
            return Task.FromResult(HealthCheckResult.Unhealthy("AeroDB daemon is stopped."));
