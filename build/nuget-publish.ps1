...
.DESCRIPTION
    ...
    GITHUB_API_KEY_AeroDB or NUGET_API_KEY environment variable (for local publishing).
.PARAMETER ApiKey
    NuGet API key to use for publishing.
    If not provided, falls back to GITHUB_API_KEY_AeroDB, then NUGET_API_KEY.
.PARAMETER SkipSnupkg
    Skip pushing symbol packages (.snupkg). Default: false.
.EXAMPLE
    # Local: uses $env:GITHUB_API_KEY_AeroDB or $env:NUGET_API_KEY
...
if ($ApiKey) {
...
} elseif (-not [string]::IsNullOrWhiteSpace($env:GITHUB_API_KEY_AeroDB)) {
    $ApiKey = $env:GITHUB_API_KEY_AeroDB
    Write-Host "Using GITHUB_API_KEY_AeroDB environment variable." -ForegroundColor Gray
    ...
    Write-Host "No API key provided. Set GITHUB_API_KEY_AeroDB or NUGET_API_KEY, or pass -ApiKey." -ForegroundColor Red
    exit 1
}
